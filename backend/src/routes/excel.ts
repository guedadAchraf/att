...
// Obtenir le dernier fichier Excel pour un formulaire
router.get('/form/:formId/latest', async (req: AuthRequest, res, next) => {
  try {
...
router.get('/:id/download', async (req: AuthRequest, res, next) => {
  try {
    ...
    // Get ALL submissions for this form (not limited to the original count)
